...
5. **F20 — Low** — Non-standard `@warning` Javadoc tag won't render correctly.
6. **F19 — Low** — `new String(name)` "defensive copy" doesn't achieve its stated purpose (`String` is immutable).
7. **F21 — Low** — `@SuppressWarnings("javadoc")` on `ChronoTask.State` instead of documenting it.
...
5. Replace `@warning` with a standard doc block; drop the pointless `String` copy. (F20, F19)
6. Add Javadoc to `ChronoTask.State`. (F21)
